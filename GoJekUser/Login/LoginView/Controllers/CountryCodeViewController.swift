...
class CountryCodeViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        searchBar.placeholder = LoginConstant.search.localized
        // self.searchBar.becomeFirstResponder()
        if pickerType  == .cityList {
            self.navigationBar.topItem?.title = LoginConstant.chooseCity.localized
        }else if pickerType == .countryList {
            self.navigationBar.topItem?.title = LoginConstant.chooseCountry.localized
        }else{
            self.navigationBar.topItem?.title = LoginConstant.chooseCountry.localized
        }
...
extension CountryCodeViewController: UISearchBarDelegate {
    ...
    func searchBar(_ searchBar: UISearchBar, textDidChange searchText: String){
        if pickerType == .countryCode {
            ...
            filterCityListEntity = cityListEntity.filter({ ($0.city_name ?? "").lowercased().contains(searchText.lowercased())})
            self.tableView.reloadData()
        }else{
            filterCountryListEntity = countryListEntity.filter({ ($0.country_name ?? "").lowercased().contains(searchText.lowercased())})
            self.tableView.reloadData()
        }
    }
